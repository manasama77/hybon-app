...
class Manufacturing1Controller extends Controller
{
    public function index()
    {
        $page_title = "Manufacturing Phase 1";
        ...
        return view('pages.manufacturing.phase-1.index', $data);
    }
    ...
    public function store_material(Request $request)
    {
        try {
            ...
            $rev = $sales_order->revisi_manufacturing_1; // edit ini
    ...
    public function destroy_material(Request $request)
    {
        try {
            ...
            $stock_monitor_id = $data->stock_monitor_id;
            $metode           = $data->metode;
            $harga_jual       = $data->price;
            $panjang          = $data->panjang;
            $lebar            = $data->lebar;
            $qty              = $data->qty;
            $kode_barang      = $data->stock_monitor->kode_barang;
            $data->deleted_by = auth()->user()->id;
            $data->save();
            $data->delete();

            $sm             = StockMonitor::find($stock_monitor_id);
            $sm->panjang    = $panjang;
            $sm->lebar      = $lebar;
            $sm->qty        = $qty;
            $sm->harga_jual = $harga_jual;
            $sm->save();
    ...
    public function update($sales_order_id, Request $request)
    {
        try {
            ...
            if (!$photo && $data->photo_manufacturing_1 == null) {
                throw new Exception('Photo manufacturing 1 Belum Diupload');
            }

            if ($photo) {
                $photoName = $data->id . '-manufacturing-1' . '.' . $photo->getClientOriginalExtension();
                $photo->move(public_path('img/manufacturing'), $photoName);
                $data->photo_manufacturing_1 = $photoName;
            }

            $data->dp    = $request->dp;
            $data->increment('revisi_manufacturing_1');
