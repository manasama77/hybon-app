...
class Manufacturing3Controller extends Controller
{
    public function index()
    {
        $page_title = "Manufacturing Phase 3";
        ...
        $datas = SalesOrder::with([
            ...
            'order_from',
            'create_name',
        ])->where('status', 'manufacturing 3')->latest()->get();
        ...
        return view('pages.manufacturing.phase-3.index', $data);
    }
    ...
    public function store_material(Request $request)
    {
        try {
            ...
            $revisi_manufacturing_3 = $sales_order->revisi_manufacturing_3;
            ...
            if ($revisi_manufacturing_3 > 0) {
                $notes = "Revisi $revisi_manufacturing_3";
            }
            ...
            $exec->notes          = $notes;
            $exec->phase_seq      = 3;
            $exec->revisi_seq     = $revisi_manufacturing_3;
    ...
    public function update($sales_order_id, Request $request)
    {
        try {
            ...
            if (!$photo && $data->photo_manufacturing_3 == null) {
                throw new Exception('Photo manufacturing 3 Belum Diupload');
            }

            if ($photo) {
                $photoName = $data->id . '-manufacturing-3' . '.' . $photo->getClientOriginalExtension();
                $photo->move(public_path('img/manufacturing'), $photoName);
                $data->photo_manufacturing_3 = $photoName;
            }

            $data->title = $request->title;
            $data->dp    = $request->dp;
            $data->increment('revisi_manufacturing_3');
