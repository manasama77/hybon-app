...
class ManufacturingInfuseController extends Controller
{
    public function index()
    {
        $page_title = "Manufacturing Infuse";
        ...
        $datas = SalesOrder::with([
            ...
            ->where('status', 'manufacturing infuse')
            ->latest()
            ->get();
        ...
        return view('pages.manufacturing.infuse.index', $data);
    }
    ...
    public function store_material(Request $request)
    {
        try {
            ...
            $revisi_manufacturing_infuse = $sales_order->revisi_manufacturing_infuse;
            ...
            if ($revisi_manufacturing_infuse > 0) {
                $notes = "Revisi $revisi_manufacturing_infuse";
            }
            ...
            $exec->notes          = $notes;
            $exec->phase_seq      = 'infuse';
            $exec->revisi_seq     = $revisi_manufacturing_infuse;
    ...
    public function update($sales_order_id, Request $request)
    {
        try {
            ...
            if (!$photo && $data->photo_manufacturing_infuse == null) {
                throw new Exception('Photo manufacturing Infuse Belum Diupload');
            }

            if ($photo) {
                $photoName = $data->id . '-manufacturing-infuse' . '.' . $photo->getClientOriginalExtension();
                $photo->move(public_path('img/manufacturing'), $photoName);
                $data->photo_manufacturing_infuse = $photoName;
            }

            $data->title = $request->title;
            $data->dp    = $request->dp;
            $data->increment('revisi_manufacturing_infuse');
