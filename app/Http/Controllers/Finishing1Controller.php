...
class Finishing1Controller extends Controller
{
    public function index()
    {
        $page_title = "Finishing 1";
        ...
        return view('pages.finishing.phase-1.index', $data);
    }
    ...
    public function store_material(Request $request)
    {
        try {
            ...
            $revisi_finishing_1 = $sales_order->revisi_finishing_1;
            ...
            if ($revisi_finishing_1 > 0) {
                $notes = "Revisi $revisi_finishing_1";
            }
            ...
            $exec->notes          = $notes;
            $exec->phase_seq      = "finishing 1";
            $exec->revisi_seq     = $revisi_finishing_1;
    ...
    public function update($sales_order_id, Request $request)
    {
        try {
            ...
            if (!$photo && $data->photo_finishing_1 == null) {
                throw new Exception('Photo finishing 1 Belum Diupload');
            }

            if ($photo) {
                $photoName = $data->id . '-finishing-1' . '.' . $photo->getClientOriginalExtension();
                $photo->move(public_path('img/finishing'), $photoName);
                $data->photo_finishing_1 = $photoName;
            }

            $data->title = $request->title;
            $data->dp    = $request->dp;
            $data->increment('revisi_finishing_1');
