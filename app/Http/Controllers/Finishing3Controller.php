...
class Finishing3Controller extends Controller
{
    public function index()
    {
        $page_title = "Finishing 3";
        ...
        return view('pages.finishing.phase-3.index', $data);
    }
    ...
    public function store_material(Request $request)
    {
        try {
            ...
            $revisi_finishing_3 = $sales_order->revisi_finishing_3;
            ...
            if ($revisi_finishing_3 > 0) {
                $notes = "Revisi $revisi_finishing_3";
            }
            ...
            $exec->notes          = $notes;
            $exec->phase_seq      = "finishing 3";
            $exec->revisi_seq     = $revisi_finishing_3;
    ...
    public function update($sales_order_id, Request $request)
    {
        try {
            ...
            if (!$photo && $data->photo_finishing_3 == null) {
                throw new Exception('Photo finishing 3 Belum Diupload');
            }

            if ($photo) {
                $photoName = $data->id . '-finishing-3' . '.' . $photo->getClientOriginalExtension();
                $photo->move(public_path('img/finishing'), $photoName);
                $data->photo_finishing_3 = $photoName;
            }

            $data->title = $request->title;
            $data->dp    = $request->dp;
            $data->increment('revisi_finishing_3');
