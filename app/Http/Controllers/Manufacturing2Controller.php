...
class Manufacturing2Controller extends Controller
{
    public function index()
    {
        $page_title = "Manufacturing Phase 2";
        ...
        $datas = SalesOrder::with([
            ...
            'order_from',
            'create_name',
        ])->where('status', 'manufacturing 2')->latest()->get();
        ...
        return view('pages.manufacturing.phase-2.index', $data);
    }
    ...
    public function store_material(Request $request)
    {
        try {
            ...
            $revisi_manufacturing_2 = $sales_order->revisi_manufacturing_2;
            ...
            if ($revisi_manufacturing_2 > 0) {
                $notes = "Revisi $revisi_manufacturing_2";
            }
            ...
            $exec->notes          = $notes;
            $exec->phase_seq      = 2;
            $exec->revisi_seq     = $revisi_manufacturing_2;
    ...
    public function update($sales_order_id, Request $request)
    {
        try {
            ...
            if (!$photo && $data->photo_manufacturing_2 == null) {
                throw new Exception('Photo manufacturing 2 Belum Diupload');
            }

            if ($photo) {
                $photoName = $data->id . '-manufacturing-2' . '.' . $photo->getClientOriginalExtension();
                $photo->move(public_path('img/manufacturing'), $photoName);
                $data->photo_manufacturing_2 = $photoName;
            }

            $data->title = $request->title;
            $data->dp    = $request->dp;
            $data->increment('revisi_manufacturing_2');
