    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#form-edit').on('submit', e => {
                ...
                $.ajax({
                    url: `/finishing-1/update/${temp_sales_order_id}`,
                    method: 'post',
                    dataType: 'json',
                    data: formData,
                    processData: false,
        ...
        function askMove(id) {
            Swal.fire({
                title: 'Pindah Data?',
                text: "Anda yakin ingin memindahkan data ini ke Finishing 2?",
                icon: 'question',
        ...
        function processMove(id) {
            $.ajax({
                url: "/sales-order/move/finishing-2/" + id,
                method: "POST",
                beforeSend: function() {
                    $.blockUI({
                        message: '<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i>',
                ...
                if (e.success) {
                    Swal.fire(
                        ...
                        if (result.isConfirmed) {
                            window.location = "{{ route('finishing-1') }}"
                        }
                    })
                } else {
                    Swal.fire(
        ...
        function showEdit(id) {
            ...
            $.ajax({
                ...
                if (e.data.photo_finishing_1) {
                    console.log(e.data.photo_finishing_1)
                    $('.img-thumbnail').attr('src', `/img/finishing/${e.data.photo_finishing_1}`)
                } else {
        ...
        function prosesTambahMaterial(id) {
            $.ajax({
                url: "{{ route('finishing-1.store_material') }}",
                method: "POST",
                dataType: 'json',
                data: {
        ...
        function getListMaterial() {
            $.ajax({
                url: `/finishing-1/show-material/${temp_sales_order_id}`,
                method: "get",
                dataType: 'json',
                beforeSend: function() {
                    $('#v_material').block({
        ...
        function destroyMaterial(id) {
            $.ajax({
                url: "{{ route('finishing-1.destroy-material') }}",
                method: "post",
                dataType: "json",
                data: {
                    id: id,
