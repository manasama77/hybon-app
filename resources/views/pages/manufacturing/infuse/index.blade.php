                                        ...
                                        <tbody>
                                            @foreach ($datas as $data)
                                                <tr>
                                                    <td>
                                                        <div class="btn-group">
                                                            @if ($data->revisi_manufacturing_infuse > 0)
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#form-edit').on('submit', e => {
                ...
                $.ajax({
                    url: `/manufacturing-infuse/update/${temp_sales_order_id}`,
                    method: 'post',
                    dataType: 'json',
                    data: formData,
                    processData: false,
        ...
        function processMove(id) {
            $.ajax({
                url: "/sales-order/move/finishing-1-infuse/" + id,
                method: "POST",
                beforeSend: function() {
                    $.blockUI({
                        message: '<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i>',
                ...
                if (e.success) {
                    Swal.fire(
                        ...
                        if (result.isConfirmed) {
                            window.location = "{{ route('manufacturing-infuse') }}"
                        }
                    })
                } else {
                    Swal.fire(
        ...
        function showEdit(id) {
            ...
            $.ajax({
                ...
                if (e.data.photo_manufacturing_infuse) {
                    console.log(e.data.photo_manufacturing_infuse)
                    $('.img-thumbnail').attr('src', `/img/manufacturing/${e.data.photo_manufacturing_infuse}`)
                } else {
        ...
        function prosesTambahMaterial(id) {
            $.ajax({
                url: "{{ route('manufacturing-infuse.store_material') }}",
                method: "POST",
                dataType: 'json',
                data: {
        ...
        function getListMaterial() {
            $.ajax({
                url: `/manufacturing-infuse/show-material/${temp_sales_order_id}`,
                method: "get",
                dataType: 'json',
                beforeSend: function() {
                    $('#v_material').block({
        ...
        function destroyMaterial(id) {
            $.ajax({
                url: "{{ route('manufacturing-infuse.destroy-material') }}",
                method: "post",
                dataType: "json",
                data: {
                    id: id,
